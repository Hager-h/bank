...
// المشروع كله عشان اعمله اخفاء واظهار
...
//عملته برا عشان خعمله كلير انترفال عشان الاتنين كونتر بتاع كل حساب ميشتغلوش سوا
//للتايمر 
let timer=function(){
let time=120
...
// عملته هنا عشان كان بيوصل لواحد ويفصل مش زيرو
--time 
//time-- ممكن كدا كمان
}
...
// جمعت كل الدوال بتاعة اظهار البيانات فى دالة عشان استدعيها براحتى
function updateUi(acco){


// وخليت اللى داخل  الكارنت اكونت واخدت منه الموفنتس والريت ///مسحت الدوا لمن فوق واستدعيتهم هنا لما اعمل لوجين
...
//لتنسيق الرصيد والان والاوت والعمله اللى فى موفنتس 
...
// للتاريح عشان يبين من يومين الامس اليوم وهكذا

let formateDate=function(datee,loca){

  let calcDay=((date1,date2)=>Math.round(Math.abs(date1-date2)/(1000*60*60*24)))
  // الفرق يسن التاريخ بتاع اناهردة والتاريخ اللى فى الاوبجكت
 let dayPassed=calcDay(new Date(), datee)
 if (dayPassed===0) return "Today"
 if (dayPassed===1) return "Yestrday"
 if (dayPassed<=7) return `${dayPassed} days ago`
 else{
// بقوله لو فرق الايام اكبر من سيعة رجع التاريخ عادى 

  // let day=`${datee.getDate()}`.padStart(2,0)
  // let year=`${datee.getFullYear()}`.padStart(2,0)
  // let month=`${datee.getMonth()+1}`.padStart(2,0)
...
//اظهار الموفمنتس اللى جوة كل اوبجكت

let displayMovement = function(acc,sort=false) {


// الترتيب
//  اعتبرت ان عندى متغير اسمه سورت الافتراضى بتاعه بخطا فهوقوله لما يبقى ترو اعمل كزا
// if sort means as if it true

  //حد نسخة عشان مغيرش فى الموفنتر الاصلية اللى داخلالى
...
//[شيك انهو اكونت والكونتر بتاعة اللى شغال]
